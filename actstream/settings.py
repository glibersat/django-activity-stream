...
def get_action_manager():
    ...
    """
    mod = SETTINGS.get("MANAGER", "actstream.managers.ActionOnSiteManager")
    mod_path = mod.split(".")
    try:
        return getattr(
            __import__(".".join(mod_path[:-1]), {}, {}, [mod_path[-1]]), mod_path[-1]
        )()
    except ImportError:
        raise ImportError(
            "Cannot import %s try fixing ACTSTREAM_SETTINGS[MANAGER]" "setting." % mod
        )
